...
function preload() {
  ...
  fonts[0] = loadFont("fonts/CourierPrime-Regular.ttf");
  fonts[1] = loadFont("fonts/CourierPrime-Bold.ttf");
  ///fonts[2] = loadFont("fonts/ZenDots-Regular.ttf");
}
...
function draw() {
  ...
  if (faces.length > 0 && faces[0].lips) {
    ...
    let bottomRightLip = createVector(faces[0].lips.x + faces[0].lips.width, faces[0].lips.y + faces[0].lips.height);
